...
class TraceableUrlMatcherTest extends TestCase
{
    ...
    {
        $routes = new RouteCollection();
        $routes->add('foo', new Route('/foo', [], [], [], 'baz', [], [], "request.headers.getValue('User-Agent') matches '/firefox/i'"));
        ...
        $this->assertEquals("Condition \"request.headers.getValue('User-Agent') matches '/firefox/i'\" does not evaluate to \"true\"", $traces[0]['log']);
